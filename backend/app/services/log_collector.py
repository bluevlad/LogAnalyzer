...
from app.models.database_models import CollectionState, LogEntry
...
# 재시작 직후 과거 로그를 얼마나 거슬러 수집할지 상한 (중복 폭주 방지)
MAX_CATCHUP_MINUTES = 60


def _load_watermarks(db: Session) -> dict[str, datetime]:
    """DB에 영속화된 컨테이너별 마지막 수집 시점 로드"""
    return {
        row.container_name: row.last_collected_at
        for row in db.query(CollectionState).all()
    }


def _save_watermark(db: Session, container_name: str, collected_at: datetime) -> None:
    """컨테이너 수집 시점 upsert"""
    state = db.get(CollectionState, container_name)
    if state:
        state.last_collected_at = collected_at
    else:
        db.add(CollectionState(
    ...
    if stripped.startswith("{"):
        try:
            ...
            if isinstance(data, dict):
                ...
                if level == "SEVERE":
                    return "ERROR"
                if level in ("FATAL", "ERROR", "INFO", "DEBUG", "TRACE"):
                    return level
        except ValueError:
            pass
...
def collect_all_logs(db: Session) -> int:
    ...
    catchup_limit = now - timedelta(minutes=MAX_CATCHUP_MINUTES)
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        ...
        for container in containers:
            since = watermarks.get(container.name, default_since)
            if since < catchup_limit:
                since = catchup_limit
        ...
        for future in futures:
            ...
            try:
                ...
                _save_watermark(db, container_name, now)
            except Exception as e:
